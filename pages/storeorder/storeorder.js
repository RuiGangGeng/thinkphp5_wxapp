...
Page({
    ...
    // 点击跳转订单详情页面
    order_details: function(e) { wx.navigateTo({ url: '/pages/shoporderdetail/shoporderdetail?id=' + e.currentTarget.dataset.id }) },
    ...
    // 点击 弹出拒绝退款输入框
    refuse_order: function(e) { this.setData({ is_refund: true, id: e.currentTarget.dataset.id }) },

    // 监听拒绝理由输入
    bindinput: function(e) { this.setData({ answer_msg: e.detail.value, }) },
    ...
    onRefuse: function() {
        ...
        let param = { id: that.data.id, answer_msg: that.data.answer_msg }
    ...
    // 取消拒绝退款
    grade_: function() { this.setData({ is_refund: false, answer_msg: '' }) },


    // 点击 拨打电话
    makeCall: function(e) { wx.makePhoneCall({ phoneNumber: e.currentTarget.dataset.phone }) },
