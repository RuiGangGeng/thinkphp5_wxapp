...
Page({
    ...
    onLoad: function(e) { this.setData({ shop_id: e.shop_id }) },

    // 监听搜索框输入
    bindinput: function(e) { this.setData({ searchKey: e.detail.value, show_button: true }) },

    // 点击搜索
    search: function(e) {
        this.setData({ page: 0, list: [], noSearch: false })
        this.loadData()
    },
    ...
    loadData: function() {
        ...
        util.wxRequest("wechat/Shop/getSearch", param, res => {
            ...
            that.data.list.length == 0 ? that.setData({ noSearch: true }) : ''
